...
const apiPath = {
  ...
  handleCreateCredential: "/api/credential",
};
...
app.post(apiPath.handleCreateCredential, (req, res) => {
  handleCreateCredential(req, res);
});

app.post(`${apiPath.handleCreateCredential}/:birthday`, (req, res) => {
  handleCreateCredential(req, res);
});
...
async function handleCreateCredential(req, res) {
  const birthday = Number(req.params.birthday) || 20030928

  console.log('birthday', birthday);
  const DOCK_API_URL = 'https://api-testnet.dock.io'
  const DOCK_API_TOKEN = 'eyJzY29wZXMiOlsidGVzdCIsImFsbCJdLCJzdWIiOiIxMDA3OCIsInNlbGVjdGVkVGVhbUlkIjoiMTQxNTAiLCJjcmVhdG9ySWQiOiIxMDA3OCIsImlhdCI6MTY5NDc2NjUwNiwiZXhwIjo0Nzc0MDYyNTA2fQ.L0SPmxX_85IML7pou_0ha-6fu6j2xgOJP04aQm2JUyQfcp1Yvi1gvsWkrJWQ5sKlUUpypbnnG0XcmPziVFMLJA'
  ...
  return res.status(200).set("Content-Type", "application/json").send(createCredentialsResponseData);
}
